...
`create_browser_data.R` calls `{tfpscanner}`, which imports the `{Rmpi}` package. MPI is a code
parallelisation tool and must be installed on your machine before `{Rmpi}` can be installed. So
before running the `create_browser_data` script.

Instructions for installing {Rmpi} can be found [here](https://www.stats.uwo.ca/faculty/yu/Rmpi/).
MPI for Windows can be obtained from [Microsoft](https://learn.microsoft.com/en-us/message-passing-interface/microsoft-mpi).

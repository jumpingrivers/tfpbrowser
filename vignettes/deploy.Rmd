...
[github](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens). If you have access to the {tfpscanner} repository on GitHub, then you will be able to install
it using your personal access token.
...
We will put the contents of the `r dirs$data` directory into the `./inst/app/www/wcdemo-data`
...
```r
dirs$embedded_data <- file.path(dirs$repo, "inst", "app", "www", "wcdemo-data")

fs::dir_copy(
  path = dirs$data,
  new_path = dirs$embedded_data
)
```
...
```r
#> ~/github/imp/tfpbrowser//inst/app/www/wcdemo-data
#> ├── mutations
#> ├── scanner_output
#> ├── sequences
#> └── treeview
...
In the `.Renviron` file, add a line that defines where the `wcdemo-data` directory is, relative to
the repository root and call this variable `APP_DATA_DIR`.

```bash
# ./.Renviron
APP_DATA_DIR=./inst/app/www/wcdemo-data
GITHUB_PAT=ghp_.....
```
...
This uploaded all the files in the repository, which is unnecessary... [TODO: continue here]
